def swapsort(array, lst=[]):
    ...
    array = list(array)
    # find max value index in the array considering same values(take latter one)
    i = max(i for i, x in reversed(list(enumerate(array))) if x == max(array))
    j = i + 1 if i < len(array) - 1 else i
    if array[i] > array[j]:
        array[j], array[i] = array[i], array[j]
        lst.append(str(i) + str(j))
        return swapsort(array, lst)
    else:
        return swapsort(array[:-1], lst)
